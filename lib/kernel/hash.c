...
/* 해시 테이블 H를 초기화하여 해시 값은 HASH를 사용하여 계산하고 해시 요소는 AUX를 사용하여 비교됩니다. */
...
/* H에서 모든 요소를 제거합니다. DESTRUCTOR가 non-NULL이면 해시의 각 요소에 대해 호출됩니다.
DESTRUCTOR는 적절하다면 해시 요소에 사용된 메모리를 해제할 수 있습니다.
그러나 DESTRUCTOR에서 또는 다른 곳에서 실행 중인 동안에는 hash_clear(), hash_destroy(), hash_insert(),
hash_replace() 또는 hash_delete() 중 하나를 사용하여 해시 테이블 H을 수정하면 정의되지 않은 동작을 초래합니다. */
...
/* 해시 테이블 H를 파괴합니다. DESTRUCTOR가 non-NULL이면 먼저 해시의 각 요소에 대해 호출됩니다.
DESTRUCTOR는 적절하다면 해시 요소에 사용된 메모리를 해제할 수 있습니다.
그러나 DESTRUCTOR에서 또는 다른 곳에서 실행 중인 동안에는 hash_clear(), hash_destroy(), hash_insert(),
hash_replace() 또는 hash_delete() 중 하나를 사용하여 해시 테이블 H을 수정하면 정의되지 않은 동작을 초래합니다. */
...
/* NEW를 해시 테이블 H에 삽입하고, 동일한 요소가 테이블에 없으면 널 포인터를 반환합니다.
...
/* 해시 테이블 H에 NEW를 삽입하고 이미 테이블에 있는 동일한 요소를 교체하고 반환합니다. */
...
/* 해시 테이블 H에서 E와 동일한 요소를 찾아 반환하거나 테이블에 동일한 요소가 없으면 널 포인터를 반환합니다. */
...
/* 해시 테이블에서 요소를 찾아 제거하고, 이를 반환합니다. 테이블에 동일한 요소가 없으면 널 포인터를 반환합니다.
만약 해시 테이블의 요소들이 동적으로 할당되었거나, 자원을 소유하고 있다면, 이는 호출자의 책임입니다.
호출자는 이들을 해제하여야 합니다. */
...
/* 해시 테이블 H의 각 요소에 대해 ACTION을 호출합니다. 순서는 임의적입니다.
hash_apply()가 실행 중일 때, hash_clear(), hash_destroy(), hash_insert(), hash_replace(), 또는 hash_delete() 등의 함수를 사용하여
해시 테이블 H를 수정하면, ACTION에서든 다른 곳에서든 정의되지 않은 동작이 발생합니다. */
...
반복 중 해시 테이블 H를 수정하면, hash_clear(), hash_destroy(), hash_insert(), hash_replace(), 
또는 hash_delete() 등의 함수를 사용하여 해시 테이블 H를 수정하면, 모든 반복자가 무효화됩니다. */
...
/* 해시 테이블에서 다음 요소로 진행하고, 해당 요소를 반환합니다. 만약 더 이상 요소가 없으면 널 포인터를 반환합니다. 
요소들은 임의적인 순서로 반환됩니다. 반복 중 해시 테이블 H를 수정하면, hash_clear(), hash_destroy(),
hash_insert(), hash_replace(), 또는 hash_delete() 등의 함수를 사용하여 해시 테이블 H를 수정하면, 모든 반복자가 무효화됩니다. */
...
/* 해시 테이블 반복 중 현재 요소를 반환하거나, 테이블의 끝에 도달하면 널 포인터를 반환합니다.
hash_first()를 호출한 후 hash_next()를 호출하기 전에 정의되지 않은 동작입니다. */
...
/* Changes the number of buckets in hash table H to match the
   ...
   이 함수는 메모리 부족으로 실패할 수 있지만, 이는 해시 접근을 덜 효율적으로 만들 뿐입니다. */
